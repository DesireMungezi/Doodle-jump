...
func generate_platforms(amount):
	...
	for items in amount:
		...
		platform_initial_position_y -= round(rand_range(50, 100))
		var normal_platform = Normal_platform.instance() as StaticBody2D
		var bounce_platform = Bounce_platform.instance() as StaticBody2D
		var break_platform = Break_platform.instance() as RigidBody2D
		var first_monster = First_monster.instance() as RigidBody2D
		...
		if randf() <= 0.7:
			...
			if randf() <= 0.15:
				first_monster.position = Vector2(paddles_position.x, paddles_position.y - 54)
				$monsters.add_child(first_monster)
		else:
			#Here 30% of the time it will be bounce and break platforms spawned 50/50 each
			if randf() <= 0.5 and break_platform.position.y - break_platform.position.y < 100:
				break_platform.position = Vector2(round(rand_range(45, 350)), platform_initial_position_y)
				$paddles.add_child(break_platform)
			else:
				bounce_platform.position = Vector2(round(rand_range(45, 350)), platform_initial_position_y)
				$paddles.add_child(bounce_platform)
